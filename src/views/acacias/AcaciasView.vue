<template>
  <div class="page-layout">
    <header class="page-header">
      <h2 class="page-title">Acacias</h2>
    </header>
    ...
    <template v-if="activeTab === 'produccion'">
      <template v-if="store.loading">
        <div class="page-state"><div class="spinner" /><span>Cargando datos de Acacias...</span></div>
      </template>
      <template v-else-if="store.error">
        <div class="page-state error">
          <span class="error-icon">!</span>
          <div><strong>Error al cargar datos</strong><p>{{ store.error }}</p></div>
          <button class="retry-btn" @click="store.fetchAcacias()">Reintentar</button>
        </div>
      </template>
      <template v-else-if="!diario.length">
        <div class="page-state"><span>No hay datos disponibles para Acacias.</span></div>
      </template>
      <template v-else>
        <div class="sticky-top">
          <header class="page-header">
            <h2 class="page-title">Agregados Producción Acacias</h2>
        ...
        <div v-else-if="prodTab === 'ordenes'" class="table-section">
          <div class="ots-section">
            ...
            <DataTable title="Producción Acacias — Detalle Diario" :data="filteredTableData" :page-size="31" :percentFields="['% Cumplimiento']" :semaphoreFields="['% Cumplimiento']" small selectColumns exportColumns :on-export="exportAcaciasXlsx" />
          </div>
        </div>
      </template>
    </template>
    ...
    <!-- Tab: Mantenimiento -->
    <EquiposDashboard v-if="activeTab === 'mantenimiento'" planta="acacias" />
  </div>
</template>
...
function navigateTo(tab: string) {
  if (tab === 'produccion') router.push('/acacias')
  else router.push(`/acacias/${tab}`)
}

const prodTab = ref('resumen')
const diario = computed(() => store.acaciasData?.rows ?? [])
...
onMounted(() => {
  if (activeTab.value === 'produccion') store.fetchAcacias()
})

watch(activeTab, (tab) => {
  if (tab === 'produccion' && !diario.value.length) store.fetchAcacias()
})
 ...
 *  Nombre del archivo: "Produccion Acacias - Mes Año". */
function exportAcaciasXlsx() {
  ...
  downloadXlsx(blob, `Produccion Acacias - ${periodo}.xlsx`)
}
