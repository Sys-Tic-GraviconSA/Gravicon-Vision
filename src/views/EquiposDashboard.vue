  ...
  <div class="page-layout">
    ...
    <div v-else>
      ...
      <div class="kpi-row">
        ...
        <KpiCard :value="otConSopledPct + '%'" label="% OT con Pedido de Almacén" accent="#10B981" icon="package" meta="OT con SOPLED (solicitud de materiales/almacén) asociada" :detail="`<div class='kpi-detail-row'><span class='kpi-dot' style='background:#3B82F6'></span><span class='kpi-label-int'>Int</span> <strong>${otsIntConSopledPct}%</strong></div><div class='kpi-detail-row'><span class='kpi-dot' style='background:#F59E0B'></span><span class='kpi-label-ext'>Ext</span> <strong>${otsExtConSopledPct}%</strong></div>`" />
      </div>
    ...
    <div class="kpi-row">
      ...
      <KpiCard :value="otsIntConSopledPct + '%'" label="% OT con Pedido de Almacén" accent="#10B981" icon="package" meta="OT con SOPLED (solicitud de materiales/almacén) asociada" :detail="`<div class='kpi-detail-row'><span class='kpi-dot' style='background:#10B981'></span><span class='kpi-label-int'>SOPLED</span> <strong>${otsIntSopled} pedidos</strong></div>`" />
    </div>
    ...
    <div class="kpi-row">
      ...
      <KpiCard :value="otsExtConSopledPct + '%'" label="% OT con Pedido de Almacén" accent="#10B981" icon="package" meta="OT con SOPLED (solicitud de materiales/almacén) asociada" :detail="`<div class='kpi-detail-row'><span class='kpi-dot' style='background:#10B981'></span><span class='kpi-label-ext'>SOPLED</span> <strong>${otsExtSopled} pedidos</strong></div>`" />
    </div>
      ...
      <div class="report-paper" v-if="informeRows.length">
        ...
        <div class="report-page">
          ...
          <div class="kpi-row compact-kpi">
            ...
            <KpiCard v-if="!isConcretos" label="Total Producción" accent="#10B981" icon="trending-up" :value="fmt(totalProd) + ' m³'" />
            ...
            <KpiCard label="Promedio OT / día" accent="#0EA5E9" icon="activity" meta="OT creadas por día con actividad" :value="promOtDia.toFixed(1)" />
            <KpiCard
              label="% Cierre"
            ...
            <KpiCard label="% OT con Pedido de Almacén" accent="#10B981" icon="package" meta="OT con SOPLED (solicitud de materiales/almacén) asociada" :value="otConSopledPct + '%'" />
...
const monthlyExpandedRange = computed<{since:number, until:number, originalSince:number} | null>(() => {
  ...
  return { since, until, originalSince }
})
...
const totalProdAgg = computed(() => {
  if (!isConcretos.value) return 0
  const raw = (concretoStore.data?.rows ?? []) as Record<string, unknown>[]
  const since = fechaInicio.value ? dateToSerial(fechaInicio.value) : -Infinity
  const until = fechaFin.value ? dateToSerial(fechaFin.value) + 1 : Infinity
  let t = 0
  for (const r of raw) {
    if (!isConcretoMezclaExcluida(r)) continue
    const v = Number(r['Fecha'])
    if (!(typeof v === 'number' && !isNaN(v) && v >= since && v < until)) continue
    t += Number(r['Cant. Concreto']) || 0
  }
  return t
})
